...
import { useOwnedNfts,useConfirmNfts } from '@hooks/web3';
...
const tabs = [
  { name: '待确认交易的通证', href: '/confirm', current: true },
  { name: '拥有的基因通证', href: '/profile', current: false },
]
...
const Confirm: NextPage = () => {
  ...
  //     consumerPubKey: "0x162165b366ae0C43B40Eb5773eF5Ced19bdDCe16",
  ...
  const decrypt = async (account: string, text: string) => {
  ...
  const confirmTx = async (nft: GeneNft) => {
    try {
      console.log(nft)

      const consumerPubKey = nft.consumerPubKey;
      const txCount = nft.txCount;
      let encryptedKey = "";
      if( txCount == 0 ){
        encryptedKey = nft.encryptedKey
      }else{
        encryptedKey = nft.consumerKey
      }

      const accounts = await ethereum?.request({method: "eth_requestAccounts"}) as string[];
      const account = accounts[0];

      const decryptKey = await decrypt(account, encryptedKey);

      console.log(decryptKey)

      const newEncryptedKey = await encrypt(consumerPubKey, decryptKey as string)

      console.log(newEncryptedKey)


      const tx = await contract?.confirmTx(
        nft.tokenId,
        newEncryptedKey
      );
      ...
      await toast.promise(
        tx!.wait(), {
          ...
          success: "Transaction has confirmed",
